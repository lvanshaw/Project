...
mainFilename = 'mainFileVehicleID.xlsx'
...
def trackMultipleObjects(file):
    ...
    mainFilename = os.path.join(directory, f"{file}.xlsx")
    ...
    carCascade = cv2.CascadeClassifier('myhaar.xml')
    video = cv2.VideoCapture('./data/'+file+'.mp4')
    ...
    while True:
        ...
        if (now.hour == 0 & now.minute == 0 & now.second == 0):
            a = 1
            counter = counter + 1
            filename = './data/output/speed/'+file+'.xlsx'
